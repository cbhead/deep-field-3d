...
- **The zsh scripts no longer run anywhere.** `test.sh`, `pr-check.sh`, `int-merge.sh`, `ci-local.sh`,
  `smoke-listen.sh` and `editor-lock.sh` are Mac-pathed. `deepfield test` already keeps `test.sh`'s
  rules; the rest are done by hand (runbook §5) until WS-15 ports them onto `deepfield.ps1`. The
