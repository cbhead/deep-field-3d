...
- **The pawn class.** `ADFGameMode` (WS-28, **active** since 2026-09-25) sets no `DefaultPawnClass` — still true and still owed, so ask WS-28, not INT, so nothing spawns a hero yet. Setting it to `ADFHeroCharacter` (or its Blueprint) needs `DFPlayer` in `DFMatch.Build.cs` (layer 4 → 3, legal; Build.cs lists are INT's).
...
- ~~**WS-28's attach PR for `UDFHeroStateComponent`** (ADR-0024; WS-28 is unclaimed, so INT per the ADR).~~ **Landed 2026-09-26** as `ea6a1d4` — `ADFPlayerState` hosts the component, the downed/revived/respawned messages go out through `ADFEventRelay`, the reviver is credited, and bled-out heroes respawn at the wave boundary. Original ask kept for the record: Create it on `ADFPlayerState` (DFMatch.Build.cs needs DFPlayer); at the intermission's end call `HostRespawn()` on each component whose `ShouldRespawnAtWaveBoundary()` and move that pawn to the hero spawn; subscribe `OnHeroLifeEvent` and publish through `ADFEventRelay`: `Downed`/`SoloDowned` → `PlayerDowned{PlayerId = seat}`, `Revived` → `PlayerRevived{PlayerId = reviver's seat, TargetPlayerId = revived seat}` plus `AddRevive()` and `AddMatchXp(5)` on the reviver (Step.cs:931), `Respawned` → `PlayerRespawned{PlayerId = seat}`. Health: set to `RevivedHpFraction` × max on `Revived` and to full on `Respawned` (the hero's health set, WS-02 attributes, once the hero has its ASC).
