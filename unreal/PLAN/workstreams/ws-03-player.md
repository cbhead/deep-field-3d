...
branch: ws/03-player/hero-state
last_commit: 4d02abe
editor_heavy: true
phase: P2
size: L
...
- **Where the hero's health lives.** `HostDeplete` needs a caller when Health reaches 0, and revive/respawn need to set it. That waits on the hero's ASC (on the pawn or the player state), which no PR has placed yet.
