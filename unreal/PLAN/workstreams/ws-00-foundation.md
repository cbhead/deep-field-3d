...
- 2026-09-25 · INT · `Source/DFMatch/**` passes to the new **WS-28 Match flow** (ADR-0023); WS-00's skeleton `ADFGameMode` is its starting point. DFCore took three contract-appends today on INT's rulings: `Determinism/DFDetRng.h` + `DFDetMath.h` moved in from DFEnemies (R2), `Online/DFJoinSeams.h` (R3), `FDFMsg_Player.OnlineId` (R3), and the `DF.Vehicle.*` leaves in `DFGameplayTagList.inl` (R10). None has been built yet — the first Mac build after landing verifies them.
