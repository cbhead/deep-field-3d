...
- 2026-09-25 · INT · INT touched one of your files on ruling R10: `DFContentTagCoverageTest.cpp` gained `{ vehicles, DF.Vehicle }` in its roots now that the four `DF.Vehicle.*` leaves are native — `DF.Content.TagCoverage` enforces the vehicle table from the next run.
