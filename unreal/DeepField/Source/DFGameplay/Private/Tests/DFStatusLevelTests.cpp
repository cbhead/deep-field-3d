...
// In the editor (unreal/Build/test.sh) AutomationOpenMap loads /Game/DF/Dev/L_Test_Status and starts
// PIE; in a -game process it travels there. The rig in the level chills and burns itself, and the
// test reads what the component, the reaction cue (through UDFMessageBus) and
// DF.Message.ReactionTriggered reported. Regenerate the level with
// Source/DFGameplay/Dev/make-l-test-status.py through the editor lock.
namespace
{
	const TCHAR* TestStatusMap = TEXT("/Game/DF/Dev/L_Test_Status");

	/** Polls the game world for the rig until it finished a cycle, then checks what the cycle showed. */
	...
	bool FDFWaitForStatusRigCommand::Update()
	{
		...
		if (Rig && Rig->CompletedCycles >= 1)
		{
			...
			Test->AddInfo(FString::Printf(TEXT("L_Test_Status (%s handled the cue): %s"), *Rig->LastCueHandlerTag.ToString(), *Rig->StateText));
			return true;
		}
