...
| `test-gate-check.py [--list]` | Every registered `DF.*` test (read from the `IMPLEMENT_*AUTOMATION_TEST` / spec macros in `Source/` and `Plugins/`) is either in the landing filter (`int-merge.sh` and `ci-local.sh`, which must agree) or excluded by a prefix in `test-gate-exclusions.tsv` with an owner and a reason (CONTRACTS/ci.md, 2026-09-21). Prints a per-suite table; `--list` prints every test and its verdict. | 1 on an ungated test, a stale or reasonless exclusion, or disagreeing filters |
