...
| `pr-check.sh [--ws NN] [--filter F] [--no-build] [--no-tests] [--smoke]` | The workstream pre-PR set (PROGRAMME.md §6.5): layering → ownership for **your** workstream (from the branch name `ws/NN-…` or `--ws`) → content schemas → editor build → the landing gate (`DF_GATE_FILTER`, the same set `int-merge` and `ci-local` run). `--filter F` runs only `F`, for iterating, and ends `pr-check: PARTIAL` rather than `OK`; so does `--no-tests`. | runbook §5.4 |
