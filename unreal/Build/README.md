...
| `ownership-check.py --ws NN\|INT [--base REF] [--files …]` | The diff `REF...HEAD` (default `origin/main`) against the glob table in `PLAN/OWNERSHIP.md`: a **binary** outside your globs is a violation, a text file outside is a warning (PR to the owner or RFC). `--ws INT` lists everything and fails nothing. | 1 on a violation |
...
| `validate-content-json.py [table\|map…]` | `unreal/content/json/*.json` against `unreal/content/schema/*.schema.json` (small in-house JSON-Schema subset), plus envelope `table` and duplicate ids; `levels/**/*.level.json` against `level.schema.json` plus unique ids, teleport legs, gate sockets, condition ids, the boss route and every `waves_<map>.json` routeId resolving to a route; `terrain/*.terrain.json` against `terrain.schema.json` plus unique ids and noise masks. A map id (`foundry`) selects every file for that map. | 1 on any failure |
...
| `plan-claim.py ws-NN --owner H --branch B` (and its renew/release/update forms, see `--help`) | Claims, renews, releases or updates a workstream: edits only that workstream file (owner, `claimed_at`, `lease_expires` +24 h, branch, state). Does not commit; the commit and the direct push to `main` are in PLAN/README.md, "Claiming in one command". | |
...
| `smoke-listen.sh [map] [-client-count N] [-map PATH] [-port P]` | A listen host on `/Game/DF/Dev/L_Dev_Empty` (or `map`) and N headless clients (default 1) that join it; OK only if every client logged `Welcomed by server` (sent after `PreLogin`'s join validators accept), the host logged host+N `player joined` with every seat above 0, and the host logged no `join refused` (re-derived 2026-09-25 against PR #48's join path; runbook §5.3). `DF_SMOKE_TIMEOUT` s (180) to wait for the joins. Logs: `Saved/Logs/smoke-host.log`, `smoke-client-<n>.log`. | `deepfield smoke [-Clients N]` (runbook §5.3) |
...
| `ci-local.sh [--skip STEP]… [--filter F] [--client-count N] [--ws ID --base REF] [--strict]` | The INT pre-merge set (§6.8) and the nightly: layering, ownership (INT view), schemas, `check-test-coverage.py`, `plan-status.py --check`, the editor build, the landing gate, the smoke. Prints a summary table on exit. A stale `STATUS.md` is a **warning** unless `--strict`: claims and lease renewals land on `main` between INT cycles by design, and a landing regenerates the ledger. | `deepfield ci-local [-Skip S] [-Strict]` (runbook §5.5) |
| `int-merge.sh <branch> [--ws NN] [--no-smoke] [--dry-run] [--resume]` (INT) | Lands one workstream branch on `main`: rebases it in the persistent verify worktree `int-verify` (serialised by `int-verify.lock`; builds stay incremental), runs the checks, builds, runs the landing gate and the smoke, commits a regenerated `STATUS.md` as its own commit, then pushes the branch and `main`. `--resume` continues a landing the worktree holds (e.g. after a hand-resolved rebase conflict). | by hand, in a throwaway worktree: rebase, `deepfield ci-local`, regenerate `STATUS.md` (`plan-status.py`), then push (never the GitHub merge button: CONTRACTS/ci.md, 2026-09-25) |
