...
| `pr-check.sh [--ws NN] [--filter F] [--no-build] [--no-tests] [--smoke]` | The workstream pre-PR set (PROGRAMME.md §6.5): layering → ownership for **your** workstream (from the branch name `ws/NN-…` or `--ws`) → content schemas → editor build → `F+DF.Content` tests (default `DF.Unit+DF.Content`; unlike `int-merge` and `ci-local` it does **not** inherit `DF_GATE_FILTER`). | runbook §5.4 (which runs the full gate) |
