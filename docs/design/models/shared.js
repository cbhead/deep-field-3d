...
P({ id: 'shared_zipline_anchor', label: 'Zipline anchor', size: '1.8 m', stats: { Interact: 'E', Cable: 'from sheave' },
  note: 'Post with a top sheave at 1.6 m — the cable leaves from `zip_sheave`, and the group declares that node in `userData.zip.sheave` so a client (or the viewer) strings the cable from the anchors themselves rather than from a second, hand-typed pair of endpoints. Brass lamp is the interact prompt anchor.',
  build(K) {
    ...
    /* The cable's attachment contract, in the same spirit as a weapon's recoil
       contract: the sheave centre in LOCAL metres, so whoever strings the line
       transforms this node and cannot invent a different end point. Local +Z
       is the direction the cable leaves in — point the anchor at its mate. */
    g.userData.zip = { sheave: [0, 1.6, .2], out: [0, 0, 1] };
    return g;
  } });
